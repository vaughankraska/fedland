...
def calc_class_distribution(x: Union[np.ndarray, List[float]]) -> float:
    x = np.asarray(x)
    diffsum = 0
...
def test():
    ...
    train_gini = calc_class_distribution(Counter(trainset.targets.numpy()))
    print(f"Train gini: {train_gini}")
    test_gini = calc_class_distribution(Counter(testset.targets.numpy()))
    print(f"Train gini: {train_gini}")
